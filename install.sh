...
# Format: id|repo|file|display_name|size_gb|ctx|min_ram_gb|min_vram_gb|tier|grade|tags|description
MODELS=(
  "1|unsloth/Qwen3.5-9B-GGUF|Qwen3.5-9B-Q4_K_M.gguf|Qwen3.5-9B-Q4_K_M.gguf (Alibaba Qwen 3.5 9B)|5.3|262144|8|6|mid|S|chat,code,reasoning|Alibaba Qwen 3.5 9B parameters · Q4_K_M quantization · Perfect all-rounder · 45-55 tok/s"
  "2|bartowski/Qwen2.5-14B-Instruct-GGUF|Qwen2.5-14B-Instruct-Q4_K_M.gguf|Qwen2.5-14B-Instruct-Q4_K_M.gguf (Alibaba Qwen2.5 14B)|8.8|131072|12|8|mid|A|chat,code,reasoning|Alibaba Qwen2.5 14B parameters · Instruct tuned · Q4_K_M quantization · Balanced performer · 35-45 tok/s"
  "3|bartowski/google_gemma-4-9b-it-GGUF|google_gemma-4-9b-it-Q4_K_M.gguf|google_gemma-4-9b-it-Q4_K_M.gguf (Google Gemma 4 9B)|5.6|8192|8|6|mid|S|chat,code|Google Gemma 4 9B parameters · Instruction tuned · Q4_K_M quantization · Latest Google · 40-50 tok/s"
  "4|bartowski/Phi-4-GGUF|Phi-4-Q4_K_M.gguf|Phi-4-Q4_K_M.gguf (Microsoft Phi-4 14B)|8.8|16384|12|8|mid|A|chat,code,reasoning|Microsoft Phi-4 14B parameters · Q4_K_M quantization · Strong reasoning · 35-45 tok/s"
  "5|bartowski/Mistral-Small-Instruct-2501-GGUF|Mistral-Small-Instruct-2501-Q4_K_M.gguf|Mistral-Small-Instruct-2501-Q4_K_M.gguf (Mistral AI Small 24B)|15.0|32768|12|10|large|A|chat,code,reasoning|Mistral AI Small 24B parameters · Instruct 2501 · Q4_K_M quantization · Efficient long context · 20-30 tok/s"
  "6|bartowski/Qwen2.5-Coder-32B-Instruct-GGUF|Qwen2.5-Coder-32B-Instruct-Q4_K_M.gguf|Qwen2.5-Coder-32B-Instruct-Q4_K_M.gguf (Alibaba Qwen2.5 Coder 32B)|20.0|131072|16|12|large|B|code|Alibaba Qwen2.5 Coder 32B parameters · Instruct tuned · Q4_K_M quantization · Elite coding model · 25-35 tok/s"
  "7|bartowski/gemma-3-27b-it-GGUF|gemma-3-27b-it-Q4_K_M.gguf|gemma-3-27b-it-Q4_K_M.gguf (Google Gemma 3 27B)|17.0|8192|16|12|large|B|chat,reasoning|Google Gemma 3 27B parameters · Instruction tuned · Q4_K_M quantization · High quality · 15-25 tok/s"
  "8|bartowski/google_gemma-4-27b-it-GGUF|google_gemma-4-27b-it-Q4_K_M.gguf|google_gemma-4-27b-it-Q4_K_M.gguf (Google Gemma 4 27B)|17.0|8192|16|12|large|B|chat,reasoning|Google Gemma 4 27B parameters · Instruction tuned · Q4_K_M quantization · Flagship model · 15-25 tok/s"
)

    # switch-model - shows real model details
    cat > "$INSTALL_DIR/switch-model" << 'EOF'
#!/usr/bin/env bash
echo "RTX 3060 12GB Optimized Models (April 2026):"
echo ""
echo "1) Qwen3.5-9B-Q4_K_M.gguf (Alibaba Qwen 3.5 9B)     - 5.3GB - S Perfect   - 45-55 t/s"
echo "2) Qwen2.5-14B-Instruct-Q4_K_M.gguf (Alibaba Qwen2.5 14B) - 8.8GB - A Excellent - 35-45 t/s"
echo "3) google_gemma-4-9b-it-Q4_K_M.gguf (Google Gemma 4 9B)   - 5.6GB - S Perfect   - 40-50 t/s"
echo "4) Phi-4-Q4_K_M.gguf (Microsoft Phi-4 14B)               - 8.8GB - A Excellent - 35-45 t/s"
echo "5) Mistral-Small-Instruct-2501-Q4_K_M.gguf (Mistral AI Small 24B) - 15GB - A Excellent - 20-30 t/s"
echo "6) Qwen2.5-Coder-32B-Instruct-Q4_K_M.gguf (Alibaba Qwen2.5 Coder 32B) - 20GB - B Good - 25-35 t/s"
echo "7) gemma-3-27b-it-Q4_K_M.gguf (Google Gemma 3 27B)       - 17GB  - B Good      - 15-25 t/s"
echo "8) google_gemma-4-27b-it-Q4_K_M.gguf (Google Gemma 4 27B) - 17GB  - B Good      - 15-25 t/s"
echo ""
echo "To switch: edit ~/.llm-config with the exact GGUF filename:"
echo "SELECTED_GGUF=\"Qwen3.5-9B-Q4_K_M.gguf\""
echo "SELECTED_NAME=\"Qwen 3.5 9B\""
echo ""
echo "Then restart: stop-llm && start-llm"
echo ""
echo "All models use Q4_K_M quantization - optimal for RTX 3060 12GB VRAM"
