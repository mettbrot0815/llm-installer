...
#  Version: production-hardened (fixed)
# =============================================================================
set -euo pipefail
...
# ── Helper: create secure temp file ───────────────────────────────────────────
# FIX: Added proper secure temp file creation (replaces hardcoded /tmp paths)
make_temp() {
    local tmpfile
    tmpfile=$(mktemp --tmpdir "llm-install.XXXXXX.${1:-tmp}") || die "Failed to create temp file"
...
if [[ -z "$_SMO" && "$HAS_NVIDIA" == "true" ]]; then
    ...
    if command -v nvcc &>/dev/null; then
        ...
        step "Installing CUDA toolkit 12.6..."
        sudo rm -f /etc/apt/trusted.gpg.d/cuda.gpg 2>/dev/null || true
        
        # FIX: Detect WSL vs native Linux and use correct repo
        if grep -qiE 'microsoft|wsl' /proc/version 2>/dev/null; then
            CUDA_REPO="wsl-ubuntu"
            ok "Detected WSL2 - using WSL CUDA repository"
        else
            CUDA_REPO="ubuntu2204"
            ok "Detected native Linux - using standard CUDA repository"
        fi
        
        CUDA_KEYRING=$(make_temp deb)
        curl -fsSL --connect-timeout 10 --max-time 60 --retry 3 --retry-delay 2 \
            ...
            -o "$CUDA_KEYRING" || die "Failed to download CUDA keyring"
...
while true; do
    ...
    # FIX: Removed useless echo -e inside read prompt
    read -rp "  ${BLD}Enter number [1-${NUM_MODELS}] or 'u' for URL:${RST} " CHOICE || {
        echo ""
        warn "EOF detected. Exiting."
        exit 0
    }
    if [[ "$CHOICE" == "u" || "$CHOICE" == "U" ]]; then
        # FIX: If download_from_hf_url fails, exit properly instead of infinite loop
        download_from_hf_url || exit 1
        break
...
if [[ -z "$_SMO" ]]; then
    ...
    if ! command -v hermes &>/dev/null || [[ ! -d "${HERMES_AGENT_DIR}/.git" ]]; then
        ...
        HERMES_INSTALL=$(make_temp sh)
        curl -fsSL --connect-timeout 15 --max-time 300 \
            https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh \
            -o "$HERMES_INSTALL" || die "Failed to download Hermes install script."
        if ! bash "$HERMES_INSTALL"; then
...
if [[ "$install_goose" =~ ^[Yy]$ ]]; then
    ...
    if command -v goose &>/dev/null; then
        ...
        GOOSE_INSTALL=$(make_temp sh)
        if curl -fsSL --connect-timeout 15 --max-time 120 --retry 3 --retry-delay 2 \
            https://github.com/block/goose/releases/download/stable/download_cli.sh \
            -o "$GOOSE_INSTALL" 2>/dev/null; then
            bash "$GOOSE_INSTALL" && export PATH="${HOME}/.local/bin:${PATH}" || \
...
# FIX: Missing closing fi for line 1038 - this was the CRITICAL BUG!
fi
