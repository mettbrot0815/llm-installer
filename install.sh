...
# Template for the launch script – quoted heredoc with scoped envsubst
...
# Export variables for scoped envsubst (only expand installer variables)
export GGUF_PATH SEL_NAME LLAMA_SERVER_BIN SAFE_CTX USE_JINJA HERMES_AGENT_DIR HERMES_VENV WORKSPACE_DIR
envsubst '${GGUF_PATH} ${SEL_NAME} ${LLAMA_SERVER_BIN} ${SAFE_CTX} ${USE_JINJA} ${HERMES_AGENT_DIR} ${HERMES_VENV} ${WORKSPACE_DIR}' \
