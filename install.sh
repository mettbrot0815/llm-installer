...
if [[ -f "$CONFIG_FILE" ]]; then
    if grep -q "^model:" "$CONFIG_FILE" 2>/dev/null; then
        ...
        cat >> "$CONFIG_FILE" <<'MODEL_EOF' | envsubst
...
MODEL_EOF
    ...
    cat > "$CONFIG_FILE" <<'CONFIG_EOF' | envsubst
...
if [[ ! -f "${HOME}/.hermes/.env" ]]; then
    ...
    mkdir -p "${HOME}/.hermes"
    cat > "${HOME}/.hermes/.env" <<'HERMES_ENV_EOF' | envsubst
...
cat > "${HOME}/.config/systemd/user/hermes-webapi.service" <<'WEBAPI_SERVICE_EOF' | envsubst
[Unit]
...
cat > "${HOME}/.config/systemd/user/hermes-workspace.service" <<'WORKSPACE_SERVICE_EOF' | envsubst
[Unit]
...
# Template for the launch script – using envsubst for safety
cat > "$LAUNCH_SCRIPT" << 'LAUNCH_EOF' | envsubst
#!/usr/bin/env bash
